...
const questionDiv = document.getElementById("questions"); //div for questions
const submitDiv = document.getElementById("submit"); //div for the submit/redo button
const resultText = document.getElementById("resultText");//h3 for results
const numQuestions = data.questions.length; //number of questions
var radioName = 1; //used for grouping radio buttons

//initialize the headings and buttons of the quiz
function initQuiz(){
    data.questions.forEach(addQuestion);
    let submitButton = document.createElement("button");
    submitButton.setAttribute("id", "submitButton");
    let text = document.createTextNode("Submit Quiz");
    submitButton.appendChild(text);
    submitDiv.appendChild(submitButton);
    submitButton.addEventListener("click", submitRedoQuiz);
    submitButton.disabled = true;
}

function submitRedoQuiz(){
    if(document.getElementById("submitButton").childNodes[0].nodeValue === "Submit Quiz"){
        document.getElementById("submitButton").childNodes[0].nodeValue ="Redo Quiz";
        let radioButtons = document.querySelectorAll('input[type="radio"]');
        let choiceArr = [];
        for(radioButton of radioButtons) {
            if(radioButton.checked){
                choiceArr.push(radioButton.value);
            }
            radioButton.disabled = true;
        }
        if(maxOccur(choiceArr) < data.results.length){
            resultText.innerText = data.results[maxOccur(choiceArr)];
        } else{
            resultText.innerText = "There number of results are not enough for the number of choices"
        }
    } else {
        document.getElementById("submitButton").childNodes[0].nodeValue ="Submit Quiz";
        let radioButtons = document.querySelectorAll('input[type="radio"]');
        for(radioButton of radioButtons) {
...
function addQuestion(value){
    ...
    div.setAttribute("name", "answerDiv");
    questionDiv.appendChild(div);
    for ([key,val] of Object.entries(value.options)) {
        createOption(key, val, div);
    }
    radioName++;
}
...
function createPrompt(question){
    ...
    heading.setAttribute("class", "prompt")
    let text = document.createTextNode(question);
    heading.appendChild(text);
    questionDiv.appendChild(heading);
}

function createOption(key, val, element){
    let button = document.createElement("input");
    button.setAttribute("class", "radio")
    button.setAttribute("type", "radio");
    button.setAttribute("name", "button"+ radioName.toString());

    //to find which button in order was clicked for the results
    if(key ==='A'){
...
function optionClick(){
    ...
    if(numButtonsChecked === numQuestions) {
        document.getElementById("submitButton").disabled = false;
    }
    
}

//finds the max occurance in an array
function maxOccur(array)
{
    if(array.length == 0) {
        return null;
    }
    let elementMap = {};
    let maxE = array[0], maxCount = 1;
    for(let i = 0; i < array.length; i++) {
        let e = array[i];
        
        if(elementMap[e] == null) {
            elementMap[e] = 1;
        } else {
            elementMap[e]++;
        }

        if(elementMap[e] > maxCount) {
            maxE = e;
            maxCount = elementMap[e];
        }
    }
    return maxE;
}
